...
const AccountSettingsPage = () => {
  ...
  // Fetch user profile data with CSRF protection
  const fetchUserProfile = async () => {
    try {
      await fetchCsrfToken();
      const response = await axios.get('routes/user-profile', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        ...
        {
          ...formData,
          userId: user.id // Include user ID from context
        },
        {
          withCredentials: true,
          headers: {
      ...
      // Clear password fields if they were changed
      ...
      if (err.response?.status === 401) {
        ...
        try {
          ...
          const retryResponse = await axios.put(
            '/ddosguard/api/routes/update-profile',
            {
              ...formData,
              userId: user.id
            },
          ...
          setSuccess(retryResponse.data.message || 'Profile updated successfully');
        } catch (retryErr) {

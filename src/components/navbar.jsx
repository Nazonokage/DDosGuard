...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try { await axios.post('/api/auth/logout'); } catch { /* no-op */ }
    logout();
    navigate('/');
    setIsOpen(false);
    ...
    <nav className="bg-[#0f172a] text-[#e0e0e0] fixed w-full shadow-md z-50 border-b border-gray-700">
            ...
            {user && (
              <div className="relative">
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className="flex items-center gap-1 hover:text-blue-400 transition-all duration-200 font-medium">
                  {user.name}
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#1e293b] border border-gray-700 rounded-md shadow-lg py-1 z-50">
                    <Link to="/account-settings"
                      className="block px-4 py-2 hover:bg-blue-500 hover:text-white transition-all"
                      onClick={() => setIsDropdownOpen(false)}>Edit Profile</Link>
                    <Link to="/feedback"
                      className="block px-4 py-2 hover:bg-blue-500 hover:text-white transition-all"
                      onClick={() => setIsDropdownOpen(false)}>Feedback</Link>
                    <button onClick={handleLogout}
                      className="block w-full text-left px-4 py-2 hover:bg-blue-500 hover:text-white transition-all">
                      Logout
                    </button>
                  </div>
                )}
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-2 pt-2 pb-3 space-y-1 bg-[#1e293b] border-t border-gray-700">
          {navLinks.map(link => (
            <Link key={link.to} to={link.to}
              className="block px-3 py-2 rounded-md text-base font-medium hover:bg-blue-500 hover:text-white transition-all"
          ...
          {user && (
            <div className="border-t border-gray-700 mt-2 pt-2">
              <Link to="/account-settings"
                ...
                onClick={() => setIsOpen(false)}>Edit Profile</Link>
              <Link to="/feedback"
                className="block px-3 py-2 rounded-md hover:bg-blue-500 hover:text-white"
                onClick={() => setIsOpen(false)}>Feedback</Link>
              <button onClick={handleLogout}

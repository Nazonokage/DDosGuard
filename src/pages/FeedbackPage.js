...
const FeedbackPage = () => {
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      try {
        const response = await axios.get('/api/feedback.php?action=index');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('/api/feedback.php?action=create', {
        message: message,
      });
      setMessage('');
      setError('');
  ...
  const handleDeleteFeedback = async (feedbackId) => {
    try {
      await axios.delete(`/api/feedback.php?action=delete&id=${feedbackId}`);
  ...
  const handleUpdateFeedback = async (feedbackId) => {
    ...
    try {
      await axios.put(`/api/feedback.php?action=update&id=${feedbackId}`, {
        message: editMessage,
      });
